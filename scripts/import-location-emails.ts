 ...
 *
 * Usage:
 *   npm run import:location-emails
 */
  ...
  try {
    ...
    for (const { market, email } of locationEmails) {
      await pool
        ...
        .query(
          `INSERT INTO dbo.location_emails (market, email) VALUES (@market, @email)`
        );
    ...
    `);
    console.log('\nSample records:');
    sampleResult.recordset.forEach((r: { market: string; email: string }) => {
