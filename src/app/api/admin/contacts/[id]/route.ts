...
async function fetchContactRow(
  ...
  return res.recordset[0] as ContactRow;
}
...
async function duplicateNameForClientExcluding(
  ...
  const res = await pool
    ...
    .input('excludeId', sql.Int, excludeContactId)
    .query(`
      SELECT 1 AS x
      FROM contacts
...
export async function GET(
  ...
  try {
    ...
    const row = await fetchContactRow(pool, id);
    if (!row) {
      return jsonError(404, 'CONTACT_NOT_FOUND');
    }

    return NextResponse.json({ success: true, data: mapContact(row) });
  } catch (error) {
    console.error('GET /api/admin/contacts/[id]:', error);
...
export async function PUT(
  ...
  try {
    ...
    const pool = await getPool();
    const existing = await fetchContactRow(pool, id);
    if (!existing) {
      return jsonError(404, 'CONTACT_NOT_FOUND');
    }
    ...
    console.error('PUT /api/admin/contacts/[id]:', error);
    return jsonError(500, 'INTERNAL_SERVER_ERROR');
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error('DELETE /api/admin/contacts/[id]:', error);
    return jsonError(500, 'INTERNAL_SERVER_ERROR');
  }
}
